...
const categories = [
  ["💰", "부업", "blue", "부업"], ["💼", "직장", "orange", "직장"], ["💕", "연애", "pink", "결혼"],
  ["💍", "결혼", "purple", "결혼"], ["🌿", "성격", "green", "성격"], ["💎", "재능", "teal", "성격"],
  ["🧠", "심리", "green", "성격"], ["💳", "돈", "indigo", "부업"], ["📈", "투자", "indigo", "부업"], ["🍕", "음식", "orange", "음식"],
] as const;
...
export default function HomePage() {
  return (
    <>
      ...
      <section id="popular-tests" className="container-page scroll-mt-20 py-16 sm:py-24">
        <SectionReveal>
          <SectionHeading icon="🔥" eyebrow="TRENDING NOW" title="지금 가장 인기 있는 테스트" description="다른 사람들은 지금 어떤 테스트에 빠져 있을까요?" href="/tests" />
          <div className="mt-9 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{rankedTests.map((test, index) => <TestCard key={test.slug} test={test} rank={index + 1} />)}</div>
        </SectionReveal>
        <AdBanner />
      </section>

      <section className="border-y border-white bg-[linear-gradient(135deg,#eff6ff_0%,#faf5ff_52%,#fff1f2_100%)]">
        <div className="container-page py-16 sm:py-24"><SectionReveal><SectionHeading icon="✨" eyebrow="JUST DROPPED" title="새로 나온 테스트" description="따끈따끈한 질문으로 새로운 나를 발견해 보세요." /><div className="mt-9 grid gap-6 sm:grid-cols-2">{tests.filter((test) => test.isNew).map((test) => <TestCard key={test.slug} test={test} />)}</div></SectionReveal></div>
      </section>
      ...
      <section className="container-page py-16 sm:py-24">
        <SectionReveal>
          ...
          <div className="mt-9 grid grid-cols-3 gap-3 sm:grid-cols-5 lg:grid-cols-9">{categories.map(([icon, label, tone, target]) => <a key={label} href={`#${target}`} className={`category-tile category-${tone}`}><span className="text-3xl" aria-hidden="true">{icon}</span><strong className="mt-2 text-sm">{label}</strong></a>)}</div>
        </SectionReveal>
      </section>
      ...
      <CategoryShowcase id="결혼" icon="💕" eyebrow="LOVE & MARRIAGE" title="연애 · 결혼 테스트" description="관계 속 나의 마음과 자연스러운 다음 타이밍을 알아보세요." test={bySlug("marriage-timing-test")} upcoming={[["💗","나의 연애 스타일 테스트","애정 표현 방식"],["🧲","나의 애착 관계 유형","가까움과 거리감"]]} tone="pink" reverse />
      <CategoryShowcase id="부업" icon="💰" eyebrow="MONEY & SIDE JOB" title="돈 · 부업 테스트" description="나의 강점과 돈을 대하는 습관을 현실적인 가능성으로 연결해 보세요." test={bySlug("side-job-recommendation")} upcoming={[["💳","나의 소비 습관 테스트","돈 관리 패턴"],["📊","나의 투자 성향 테스트","위험과 안정 선호"]]} tone="blue" />
      <CategoryShowcase id="성격" icon="🧠" eyebrow="PERSONALITY" title="성격 · 재능 테스트" description="아직 이름 붙이지 못한 성향과 숨은 재능을 발견해 보세요." upcoming={[["💎","나의 숨은 재능 찾기","강점과 몰입 영역"],["🌿","나는 내향형일까 외향형일까?","에너지 충전 방식"],["🧩","나의 문제 해결 스타일","사고와 판단 방식"]]} tone="green" reverse />
      ...
      <section className="border-y border-slate-200/70 bg-white">
        <div className="container-page grid gap-10 py-16 lg:grid-cols-[.9fr_1.1fr] lg:items-center sm:py-24">
          ...
          <SectionReveal className="rounded-[2rem] bg-slate-950 p-4 shadow-2xl shadow-slate-300 sm:p-6"><ol className="space-y-3">{rankedTests.map((test, index) => <li key={test.slug}><Link href={`/tests/${test.slug}`} className="flex min-h-20 items-center gap-4 rounded-2xl border border-white/10 bg-white/[.06] p-4 transition hover:bg-white/[.11]"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-white/10 text-xl">{["🥇","🥈","🥉"][index] ?? index + 1}</span><span className="min-w-0 flex-1"><strong className="block truncate text-sm text-white sm:text-base">{test.cardTitle ?? test.title}</strong><span className="mt-1 block text-xs text-slate-400">{test.type === "worldcup" ? `${test.itemCount}강` : `${test.questions.length}문항`} · {test.duration}</span></span><span className="shrink-0 text-right"><strong className="block text-sm text-white">{test.participants.toLocaleString("ko-KR")}</strong><span className="text-[10px] text-slate-500">명 참여</span></span></Link></li>)}</ol></SectionReveal>
        </div>
      </section>

      <section className="container-page py-16 sm:py-24"><SectionReveal><SectionHeading icon="💬" eyebrow="SAMPLE REVIEW" title="이런 반응이 나오는 테스트" description="결과를 읽는 순간 ‘어, 진짜 나 같은데?’라는 느낌을 목표로 만들었어요." /><div className="mt-9 grid gap-4 sm:grid-cols-3">{[["“친구랑 같이 했는데 서로 결과가 너무 딱 맞아서 한참 웃었어요.”","직장 동물 테스트"],["“막연했던 부업 후보가 세 개로 좁혀져서 바로 하나 시작해 보려고요.”","부업 추천 테스트"],["“예언처럼 단정하지 않고 지금 제 상황을 설명해 줘서 더 공감됐어요.”","결혼 시기 테스트"]].map(([quote, test]) => <figure key={test} className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-card"><div className="text-amber-400" aria-label="별점 5점">★★★★★</div><blockquote className="mt-4 text-sm font-semibold leading-7 text-slate-700">{quote}</blockquote><figcaption className="mt-5 border-t border-slate-100 pt-4 text-xs font-bold text-slate-400">{test} · 후기 예시</figcaption></figure>)}</div></SectionReveal></section>

      <section className="border-t border-slate-200 bg-white"><div className="container-page py-16 sm:py-24"><SectionReveal><SectionHeading icon="📚" eyebrow="TEST GUIDE" title="테스트 가이드" description="재미로 발견하고, 현실적인 정보로 한 걸음 더 나아가세요." href="/blog" /><div className="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{blogPosts.slice(0, 3).map((post) => <BlogCard key={post.slug} post={post} />)}</div></SectionReveal></div></section>
...
type Tone = "blue" | "orange" | "pink" | "green";
