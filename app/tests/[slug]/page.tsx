...
export default async function TestDetailPage({ params, searchParams }: Props) {
  ...
  const answerType=test.type==="likert"||isBurnoutRisk||isBigFive?"5점 척도":isKkondaePower||isEnneagram||isEqTest?"4지선다":isJoseonDestiny||isPersonalityCountry||isLoverScore||isColorPersonality?"2지선다":"O/X";
...
function BigFiveSeoContent() {
  return (
    <section className="mx-auto mt-8 max-w-3xl rounded-3xl border border-indigo-100 bg-white p-6 shadow-card sm:p-8">
      <h2 className="text-2xl font-black text-ink">Big Five 성격 테스트란?</h2>
      <div className="mt-4 space-y-5 text-sm leading-8 text-slate-700">
        <p>Big Five 성격 테스트는 현대 심리학에서 가장 널리 사용되는 성격 분석 모델 중 하나입니다. OCEAN 성격 테스트라고도 부르며, 사람을 하나의 고정된 유형으로 나누기보다 여러 성격 특성이 어느 정도로 나타나는지 연속적인 차원으로 살펴봅니다.</p>
        <p>OCEAN은 개방성(Openness), 성실성(Conscientiousness), 외향성(Extraversion), 친화성(Agreeableness), 정서 민감성(Neuroticism)의 앞 글자를 딴 이름입니다. 개방성은 새로운 경험과 아이디어에 대한 호기심, 성실성은 계획성과 책임감, 외향성은 사람과 활동에서 얻는 에너지, 친화성은 공감과 협력 성향, 정서 민감성은 감정 변화와 스트레스 신호에 대한 민감도를 의미합니다.</p>
        <p>미미테스트의 빅파이브 테스트는 25개의 일상 문항을 통해 5개 성격 차원을 0~100점으로 환산합니다. 결과에서는 레이더 차트와 함께 업무 스타일, 인간관계 스타일, 연애 스타일, 스트레스 반응 방식, 성장 포인트를 함께 제공합니다. 특정 점수가 높거나 낮다고 해서 좋고 나쁨을 판단하지 않고, 각 성향이 어떤 상황에서 강점이 되고 어떤 부분을 보완하면 좋은지 쉽게 이해할 수 있도록 구성했습니다.</p>
      </div>
      <JsonLd data={{ "@context":"https://schema.org", "@type":"WebPage", name:"Big Five 성격 테스트", description:"OCEAN 성격 모델을 기반으로 한 무료 성격 분석 테스트", url:absoluteUrl("/tests/big-five"), inLanguage:"ko-KR", mainEntity:{ "@type":"Quiz", "@id":absoluteUrl("/tests/big-five#quiz") } }} />
      <JsonLd data={{ "@context":"https://schema.org", "@type":"Quiz", "@id":absoluteUrl("/tests/big-five#quiz"), name:"Big Five 성격 테스트", description:"OCEAN 성격 모델을 기반으로 한 무료 성격 분석 테스트", url:absoluteUrl("/tests/big-five"), inLanguage:"ko-KR", assesses:"OCEAN Big Five 성격 차원", educationalUse:"assessment", interactivityType:"active", isAccessibleForFree:true }} />
    </section>
  );
}
