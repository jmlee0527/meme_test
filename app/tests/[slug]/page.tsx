...
export default async function TestDetailPage({ params, searchParams }: Props) {
  ...
  if (isNctDreamFan) return <FanQuizLanding test={test} insight="NCT DREAM 멤버와 데뷔, 앨범, 수록곡, 일본 활동, THE DREAM SHOW를 공식 자료로 검증한 60문항 중 easy 5문항·medium 5문항·hard 5문항을 출제합니다. 동일 사실은 반복하지 않고 topicKey와 수록곡 문제, 카테고리 구성을 균형 있게 조정합니다. 결과에서는 30점 만점의 가중 점수와 정답 수, 시즈니 팬심 지수, 문제별 해설을 제공합니다. 본 테스트는 비공식 팬 퀴즈입니다." />;
  if (isBigbangFan && start === "1") return <BigbangFanQuizTestPage />;
  if (isBigbangFan) return <FanQuizLanding test={test} insight="BIGBANG의 데뷔와 대표곡, 앨범 수록곡, MADE SERIES, 공연 기록을 공식 자료로 검증한 60문항 중 easy 5문항·medium 6문항·hard 4문항으로 출제합니다. 동일 topicGroup은 한 회차에 한 번만 포함하고 카테고리 편중도 제한합니다. 결과에서는 15점 만점 점수와 정답률, VIP 등급, 문제별 해설을 확인할 수 있습니다. 본 테스트는 비공식 팬 퀴즈입니다." />;
  if (isGirlsGenerationFan && start === "1") return <GirlsGenerationFanQuizTestPage />;
  if (isGirlsGenerationFan) return <FanQuizLanding test={test} insight="소녀시대 멤버와 데뷔, 다시 만난 세계·Gee·소원을 말해봐 같은 대표곡, 정규앨범과 FOREVER 1, 태티서와 Oh!GG 유닛 활동까지 공식 자료로 검증한 60문항 문제은행에서 easy 5문항·medium 6문항·hard 4문항이 무작위 출제됩니다. 같은 분야와 같은 앨범에 문제가 몰리지 않도록 제한하며, 정답과 해설은 제출 후 결과 화면에서 공개됩니다. 결과에서는 15점 만점 점수와 정답률, 5단계 S♡NE 등급, 문제별 해설 복습을 제공합니다. 본 테스트는 공식 서비스가 아닌 비공식 팬 퀴즈입니다." />;
  const isResceneFan = test.slug === "rescene-true-fan-test";
  if (isResceneFan && start === "1") return <ResceneFanQuizTestPage />;
  if (isResceneFan) return <FanQuizLanding test={test} insight="RESCENE 멤버와 데뷔, LOVE ATTACK, SCENEDROME, Glow Up, lip bomb과 향기 콘셉트를 공식 자료로 검증한 60문항 중 easy 5문항·medium 6문항·hard 4문항으로 출제합니다. 동일 카테고리와 albumKey, 최신성 문항의 편중을 제한하며 결과에서는 점수와 REMINE 등급, 문제별 해설을 확인할 수 있습니다." />;
  if (isAteezFan && start === "1") return <AteezFanQuizTestPage />;
  if (isAteezFan) return <FanQuizLanding test={test} insight="ATEEZ 멤버와 데뷔, 앨범, 타이틀곡, 수록곡과 유닛 활동을 공식 자료로 검증한 60문항 중 easy 5문항·medium 6문항·hard 4문항을 출제합니다. 동일 카테고리와 음반·시리즈, 최신성 문항이 한쪽에 몰리지 않도록 제한하며 결과에서는 점수와 정답률, ATINY 등급과 문제별 해설을 확인할 수 있습니다." />;
  if (isManchesterUnitedFan && start === "1") return <ManchesterUnitedFanQuizTestPage />;
  if (isManchesterUnitedFan) return <FanQuizLanding test={test} insight="뉴턴 히스 시절부터 올드 트래포드, 버스비 베이브스, 1999년 트레블과 유럽 정상의 순간까지 공식 자료로 검증한 60문항 중 easy 5문항·medium 6문항·hard 4문항을 출제합니다. 최소 6개 카테고리와 5개 시대를 포함하도록 구성하며 결과에서는 정답 수, 29점 만점의 가중 점수와 맨유 팬심 백분율을 확인할 수 있습니다." />;
  if (isStrayKidsFan && start === "1") return <StrayKidsFanQuizTestPage />;
  if (isStrayKidsFan) return <FanQuizLanding test={test} insight="스트레이 키즈 멤버와 앨범, 타이틀곡, 수록곡과 유닛 활동을 공식 공개 자료로 검증한 180문항 중 easy 5문항·medium 6문항·hard 4문항이 출제됩니다. 같은 앨범과 카테고리, 최신 연도 문제의 편중을 제한하며 결과에서는 정답 수, 29점 만점의 가중 점수와 STAY 팬심 백분율을 확인할 수 있습니다. 본 테스트는 비공식 팬 퀴즈입니다." />;
  if (isSeventeenFan && start === "1") return <SeventeenFanQuizTestPage />;
  if (isSeventeenFan) return <FanQuizLanding test={test} insight="세븐틴 멤버와 유닛, 대표곡, 앨범과 수록곡까지 공식 공개 자료로 검증한 60문항 중 초급 5문항·중급 6문항·고급 4문항이 출제됩니다. 같은 시도에서는 문제와 보기 순서가 유지되고, 다시 도전하면 새로운 조합으로 나의 캐럿력을 확인할 수 있습니다. 본 테스트는 미미테스트가 자체 제작한 비공식 팬 퀴즈입니다." />;
  if (isFromis9Fan && start === "1") return <Fromis9FanQuizTestPage />;
  if (isFromis9Fan) return <FanQuizLanding test={test} insight="나는 프로미스나인을 얼마나 알고 있을까요? 아이돌학교와 '프로미스의 방'에서 시작된 데뷔 스토리부터 To. Heart와 두근두근, DM, Supersonic까지의 디스코그래피, 첫 단독 콘서트 LOVE FROM., 음악방송 1위 기록과 최신 5인 체제 활동까지 60개 문제은행에서 매번 15문제가 랜덤 출제됩니다. 난이도는 쉬움 5문제·보통 5문제·어려움 5문제로 항상 균형이 유지되며, 모든 문제는 공식 발표와 공개 기록으로 사실 검증을 거쳤습니다. 결과에서는 6단계 팬심 등급과 분야별 정답률, 해설이 담긴 오답노트를 확인할 수 있습니다. 본 테스트는 프로미스나인 또는 소속사의 공식 서비스가 아닌 비공식 팬 퀴즈입니다." />;
  ...
  if (isYoungtakFan && start !== "1") return <FanQuizLanding test={test} insight="나는 영탁을 얼마나 잘 알고 있을까요? 노래, 앨범, 방송, 무대, 콘서트, 작사·작곡 참여와 활동 기록까지 총 60개의 문제은행에서 난이도별 15문항이 랜덤으로 출제됩니다. 쉬움 4문항·보통 6문항·어려움 4문항·최상 1문항으로 총점은 정확히 100점이며, 결과에서는 카테고리별 정답률과 오답노트를 확인할 수 있습니다. 본 테스트는 영탁 또는 소속사의 공식 서비스가 아닌 비공식 팬 퀴즈입니다." />;
  if (isLimYoungWoongFan && start !== "1") return <FanQuizLanding test={test} insight="나는 임영웅을 얼마나 잘 알고 있을까요? 데뷔, 노래, 정규앨범, 경연, 방송, OST, 콘서트와 제작 크레딧까지 공식·주요 음원 자료로 검증한 60개 문제은행에서 난이도별 15문항이 랜덤 출제됩니다. 쉬움 4문항·보통 6문항·어려움 4문항·최상 1문항으로 총점은 정확히 100점이며, 결과에서는 분야별 정답률과 오답노트를 확인할 수 있습니다. 본 테스트는 임영웅 또는 소속사의 공식 서비스가 아닌 비공식 팬 퀴즈입니다." />;
  if (isArsenalFan && start !== "1") return <FanQuizLanding test={test} insight="나는 아스날을 얼마나 잘 알고 있을까요? 선수와 감독, 우승 기록, 홈구장, 유럽대항전, 역사적인 경기까지 총 15개의 문제를 풀고 나의 아스날 팬 지수를 확인해 보세요. 문제은행은 50문항 이상으로 구성되어 있으며, 매번 초급 5문항·중급 6문항·고급 4문항이 무작위 출제됩니다. 본 테스트는 아스날 FC 또는 프리미어리그의 공식 서비스가 아닌 비공식 팬 퀴즈입니다." />;
