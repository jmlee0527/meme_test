...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return createMetadata({
    title: `${profile.code} ${profile.name} | 16가지 성격 유형 테스트 결과`,
    description: `${profile.tagline}. ${profile.code} 유형의 성격 특징, 강점과 주의할 점, 어울리는 직업과 관계 팁을 확인해보세요.`,
    path: `/tests/mbti/result/${profile.slug}`,
    keywords: [profile.code, `${profile.code} 특징`, `${profile.code} 성격`, profile.name, "성격유형 테스트", "MBTI 스타일 테스트", "16가지 성격 유형"],
    ogImage: false,
  });
}
    ...
    <>
      ...
      <JsonLd
        data={{
          ...
          "@type": "WebPage",
          name: `성격 유형 테스트 결과: ${profile.code} ${profile.name}`,
