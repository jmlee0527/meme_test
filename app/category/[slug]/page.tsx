...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return createMetadata({ title: `${category} 테스트와 가이드`, description: `${category}에 관한 무료 테스트와 현실적인 시작 가이드를 모아보세요.`, path: `/category/${encodeURIComponent(category)}`, keywords: [`${category} 부업`, `${category} 가이드`] });
}
