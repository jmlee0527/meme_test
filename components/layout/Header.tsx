    ...
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
      <div className="container-page flex min-h-[4.25rem] items-center justify-between py-2">
        ...
        <nav className="flex items-center gap-0.5 text-xs font-semibold text-slate-600 sm:gap-1 sm:text-sm" aria-label="주요 메뉴">
          <Link onClick={() => trackNav("home")} aria-current={current("home") ? "page" : undefined} className="rounded-lg px-2 py-2 transition hover:bg-slate-100 hover:text-ink sm:px-3" href="/">
            홈
          </Link>
          ...
          <Link onClick={() => trackNav("columns")} aria-current={current("columns") ? "page" : undefined} className="rounded-lg px-2 py-2 transition hover:bg-slate-100 hover:text-ink aria-[current=page]:bg-slate-100 aria-[current=page]:text-ink sm:px-3" href="/blog">
            칼럼
          </Link>
