...
import { BTS_FAN_QUIZ_SIZE, btsFanGradeProfiles } from "@/data/bts-fan";
...
const difficultyRows = [
  { key: "easy", label: "하 (입문)", total: 5, color: "bg-slate-500" },
  { key: "medium", label: "중 (팬)", total: 5, color: "bg-purple-500" },
  { key: "hard", label: "상 (찐팬)", total: 5, color: "bg-violet-700" },
] as const;

export function BtsFanQuizResult({ grade, score, easyCorrect, mediumCorrect, hardCorrect, wrong, categoryRates, encodedAnswers }: Props) {
        ...
        <div className="mx-auto max-w-4xl">
          <SectionReveal>
            <section className="relative overflow-hidden rounded-[2.25rem] bg-[#1e1b4b] text-center text-white shadow-2xl">
              ...
              <div className="relative px-6 pb-8 pt-10 sm:px-10 sm:pt-14">
                ...
                <div className="mx-auto mt-6 grid size-40 place-items-center rounded-full p-2 shadow-lg" style={{ background: `conic-gradient(#8b5cf6 ${((hasResult ? displayScore : grade.maxScore) / BTS_FAN_QUIZ_SIZE) * 100}%, rgba(255,255,255,.15) 0)` }}>
                  <div className="grid size-full place-items-center rounded-full bg-[#1e1b4b]">
                    <span>
                      {hasResult ? (
                        <strong className="block text-5xl font-black text-white">{displayScore}<span className="text-xl text-violet-300">/{BTS_FAN_QUIZ_SIZE}</span></strong>
                      ) : (
                        <strong className="block text-2xl font-black text-white">{grade.minScore === grade.maxScore ? `${grade.maxScore}점` : `${grade.minScore}~${grade.maxScore}점`}</strong>
                      )}
                      <span className="text-[11px] font-bold text-violet-300/80">ARMY INDEX</span>
                    </span>
                  </div>
                </div>
                ...
                {hasResult && (
                  <div className="mx-auto mt-6 grid max-w-lg grid-cols-2 gap-3 sm:grid-cols-4">
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/10 p-3"><span className="block text-xs text-violet-200">하</span><strong className="text-xl">{easyCorrect}/5</strong></div>
                    <div className="rounded-2xl border border-white/10 bg-white/10 p-3"><span className="block text-xs text-violet-200">중</span><strong className="text-xl">{mediumCorrect}/5</strong></div>
                    <div className="rounded-2xl border border-white/10 bg-white/10 p-3"><span className="block text-xs text-violet-200">상</span><strong className="text-xl">{hardCorrect}/5</strong></div>
                  </div>
                )}
              </div>
                ...
                {wrong.map(({ question, choice }, index) => (
                  <details key={question.id} className="group rounded-2xl border border-slate-100 bg-slate-50/70 p-4 open:bg-white">
                    ...
                    <dl className="mt-4 space-y-2 text-sm">
                      ...
                      <div className="flex gap-2"><dt className="shrink-0 font-black text-slate-500">분야</dt><dd className="text-slate-600">{question.category} · 난이도 {question.difficulty === "easy" ? "하" : question.difficulty === "medium" ? "중" : "상"}</dd></div>
                    </dl>
          ...
          {hasResult && wrong.length === 0 && (
            <section className="mt-8 rounded-3xl border border-violet-100 bg-violet-50/70 p-6 text-center shadow-card sm:p-8">
              ...
              <p className="mt-2 text-sm leading-6 text-violet-800">상 난이도까지 모두 해결했습니다. 이 정도면 공식 콘텐츠 아카이브 담당자급 레전드 ARMY입니다.</p>
            </section>
          )}
          ...
          <section id="share-card" className="mt-10 grid scroll-mt-24 gap-6 rounded-3xl bg-ink p-6 text-white sm:p-8 lg:grid-cols-[.8fr_1.2fr] lg:items-center">
            <ShareImageCard emoji={grade.icon} eyebrow="BTS 찐팬 테스트" title={`${shownScore}점 · ${grade.name}`} subtitle={grade.subtitle} badge={`${shownScore}/${BTS_FAN_QUIZ_SIZE}점`} accent="purple" />
            <div>
