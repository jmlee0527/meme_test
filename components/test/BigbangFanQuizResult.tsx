...
export function BigbangFanQuizResult({ answers }: { answers: BigbangAnswer[] | null }) {
  ...
  const sharePath = `/bigbang-true-fan-test/result/${result.grade.slug}?r=${encoded}`;
  ...
  const shareText = `나는 빅뱅 찐팬 테스트 ${level}레벨! 15문제 중 ${result.score}문제 정답, 내 VIP 팬 지수는 ‘${result.grade.name}’`;

  const retry = () => { window.sessionStorage.removeItem("mimi-bigbang-true-fan-test-session"); window.location.href = "/tests/bigbang-true-fan-test?start=1"; };

  return <main className="min-h-screen bg-[radial-gradient(circle_at_top,#ede9fe_0,#fff7ed_40%,#f8fafc_100%)] pb-24 pt-8 sm:py-14">
    <div className="container-page"><Breadcrumbs items={[{ name: "테스트", href: "/tests" }, { name: "빅뱅 찐팬 테스트", href: "/tests/bigbang-true-fan-test" }, { name: result.grade.name }]} />
      <div className="mx-auto max-w-4xl">
        <section className="overflow-hidden rounded-[2.25rem] border border-violet-100 bg-white text-center shadow-2xl shadow-violet-100/70">
          <div className="bg-gradient-to-br from-violet-600 via-fuchsia-500 to-rose-400 px-6 py-10 text-white sm:py-14"><p className="text-sm font-bold text-white/80">나의 VIP 팬 지수 결과</p><div className="mt-5 text-6xl">{result.grade.icon}</div><h1 className="mt-3 text-4xl font-black sm:text-5xl">{result.grade.name}</h1><p className="mx-auto mt-4 max-w-xl font-semibold leading-7 text-white/90">{result.grade.summary}</p></div>
