...
import { SBTI_QUESTION_COUNT, calculateSbtiResult, encodeSbtiLevels } from "@/lib/sbti-engine";
  ...
  const select = (choice: number) => {
    ...
    window.setTimeout(() => {
      ...
      if (nextAnswers.length >= SBTI_QUESTION_COUNT) {
        ...
        router.push(`/tests/sbti/result/${result.profile.slug}?d=${encodeSbtiLevels(result.levels)}&m=${result.match}`);
      } else {
      ...
      <div className="container-page mx-auto max-w-2xl">
        <header className="mb-6">
          <div className="flex items-end justify-between gap-4">
            <div>
              <p className="text-xs font-black tracking-[.16em] text-rose-500">SBTI TEST</p>
              <h1 className="mt-2 text-xl font-black text-ink sm:text-2xl">SBTI 테스트</h1>
            </div>
        ...
        <AnimatePresence mode="wait">
          <motion.section key={current.id} initial={reduceMotion ? false : { opacity: 0, x: 28 }} animate={{ opacity: 1, x: 0 }} exit={reduceMotion ? undefined : { opacity: 0, x: -24 }} transition={{ duration: 0.2 }} className="rounded-[2rem] border border-white/90 bg-white/90 p-6 shadow-2xl shadow-rose-100/60 backdrop-blur sm:p-10">
            <span className="rounded-full bg-rose-50 px-3 py-1.5 text-xs font-black text-rose-600">Q{index + 1}</span>
            ...
            <div className="mt-8 grid gap-2.5" role="radiogroup" aria-label="선택지">
              {current.options.map((option, optionIndex) => (
                <motion.button
                  ...
                  className={`flex min-h-14 items-center gap-4 rounded-2xl border px-5 text-left text-sm font-bold leading-6 transition sm:text-base ${answers[index] === optionIndex ? "border-rose-400 bg-rose-50 text-rose-900 shadow-md" : "border-slate-200 bg-white text-slate-700 hover:border-rose-300 hover:bg-rose-50/50"}`}
                >
                  <span className={`grid size-8 shrink-0 place-items-center rounded-full text-xs font-black ${answers[index] === optionIndex ? "bg-rose-500 text-white" : "bg-slate-100 text-slate-500"}`}>{badges[optionIndex]}</span>
                  {option.text}
                </motion.button>
              ))}
            </div>
