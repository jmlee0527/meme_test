...
export function StrayKidsFanQuizResult({ answers }: { answers: StrayKidsAnswer[] | null }) {
  ...
  const shareText = `나는 스트레이 키즈 찐팬 테스트 ${level}레벨! 15문제 중 ${result.score}문제 정답, 내 STAY 팬심은 ‘${result.grade.name}’`;

  const track = (name: string) => (window as Window & { gtag?: (command: string, event: string) => void }).gtag?.("event", name);
  const retrySame = () => { const key = "mimi-stray-kids-true-fan-session"; const stored = JSON.parse(window.sessionStorage.getItem(key) ?? "{}"); window.sessionStorage.setItem(key, JSON.stringify({ seed: stored.seed, answers: [], index: 0, restartMode: "same" })); track("stray_kids_fan_retry_same"); window.location.href = "/tests/stray-kids-true-fan-test?start=1"; };
