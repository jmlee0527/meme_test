...
export function SeventeenFanQuizResult({ answers }: { answers: SeventeenAnswer[] | null }) {
  ...
  const sharePath = `/seventeen-true-fan/result/${result.grade.slug}?r=${encoded}`;
  ...
  const shareText = `나는 세븐틴 찐팬 테스트 ${level}레벨! 15문제 중 ${result.score}문제 정답, 내 캐럿력은 ‘${result.grade.name}’`;

  const retry = () => { window.sessionStorage.removeItem("mimi-seventeen-true-fan-session"); window.location.href = "/tests/seventeen-true-fan?start=1"; };

  return <main className="min-h-screen bg-[radial-gradient(circle_at_top,#ede9fe_0,#fff7ed_40%,#f8fafc_100%)] pb-24 pt-8 sm:py-14">
    <div className="container-page"><Breadcrumbs items={[{ name: "테스트", href: "/tests" }, { name: "세븐틴 찐팬 테스트", href: "/tests/seventeen-true-fan" }, { name: result.grade.name }]} />
      <div className="mx-auto max-w-4xl">
        <section className="overflow-hidden rounded-[2.25rem] border border-violet-100 bg-white text-center shadow-2xl shadow-violet-100/70">
          <div className="bg-gradient-to-br from-violet-600 via-fuchsia-500 to-rose-400 px-6 py-10 text-white sm:py-14"><p className="text-sm font-bold text-white/80">나의 캐럿력 결과</p><div className="mt-5 text-6xl">{result.grade.icon}</div><h1 className="mt-3 text-4xl font-black sm:text-5xl">{result.grade.name}</h1><p className="mx-auto mt-4 max-w-xl font-semibold leading-7 text-white/90">{result.grade.description}</p></div>
        ...
        <section className="mt-7 rounded-3xl border border-slate-200 bg-white p-6 shadow-card sm:p-8"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-black text-ink">{showAll ? "전체 정답 보기" : "틀린 문제 해설"}</h2><button type="button" onClick={() => setShowAll((value) => !value)} className="min-h-11 rounded-xl border border-violet-200 px-4 text-sm font-bold text-violet-700">{showAll ? "오답만 보기" : "전체 정답 펼치기"}</button></div>
          {shownReviews.length ? <div className="mt-5 space-y-3">{shownReviews.map((review, index) => <details key={review.question.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><summary className="cursor-pointer font-extrabold text-ink"><span className={review.correct ? "text-emerald-600" : "text-rose-500"}>{review.correct ? "정답" : "오답"}</span> · {review.question.prompt}</summary><dl className="mt-4 grid gap-2 text-sm"><div><dt className="inline font-black text-slate-500">내 답: </dt><dd className="inline">{review.question.choices[review.choice]}</dd></div><div><dt className="inline font-black text-emerald-600">정답: </dt><dd className="inline font-bold">{review.question.answerText}</dd></div></dl><p className="mt-4 border-t border-slate-200 pt-4 text-sm leading-6 text-slate-600">{review.question.explanation}</p><p className="mt-2 text-xs text-slate-400">검증 기준: {review.question.verifiedAt.replaceAll("-", ".")}</p></details>)}</div> : <p className="mt-5 rounded-2xl bg-emerald-50 p-5 text-sm font-bold text-emerald-800">15문제를 모두 맞혔습니다!</p>}
        </section>

        <section id="share-card" className="mt-8 rounded-3xl bg-slate-950 p-6 text-white sm:p-8"><h2 className="text-2xl font-black">내 캐럿력 공유하기</h2><p className="mt-2 text-sm leading-6 text-slate-300">{shareText}</p><div className="mt-5"><ShareButtons title={shareText} description="당신의 캐럿력도 확인해 보세요." path={sharePath} /></div></section>
